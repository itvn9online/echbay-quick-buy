(function ($) {
  ...
  function isCaptchaEnabled() {
    return (
      eqb_vars.captcha &&
      eqb_vars.captcha.provider &&
      eqb_vars.captcha.provider !== "off" &&
      eqb_vars.captcha.site_key
    );
  }

  function isRecaptchaV3() {
    return (
      isCaptchaEnabled() && eqb_vars.captcha.provider === "google_recaptcha_v3"
    );
  }

  function isCaptchaWidgetProvider() {
    return isCaptchaEnabled() && !isRecaptchaV3();
  }
  ...
  function initCaptcha($form) {
    ...
    var $container = $form.find("[data-eqb-captcha]");
    if (!$container.length) {
      return;
    }
    ...
    if (isRecaptchaV3()) {
      $container.html(
        '<p class="eqb-captcha-v3-notice">' +
          eqb_vars.i18n.recaptchaV3Notice +
          "</p>",
      );
      return;
    }
    ...
    function tryRender(attemptsLeft) {
      if (
        eqb_vars.captcha.provider === "google_recaptcha" &&
        typeof grecaptcha !== "undefined" &&
        typeof grecaptcha.render === "function"
      ) {
        captchaWidgetId = grecaptcha.render($container[0], {
          sitekey: eqb_vars.captcha.site_key,
        });
        return;
      }

      if (
        eqb_vars.captcha.provider === "cloudflare_turnstile" &&
        typeof turnstile !== "undefined" &&
        typeof turnstile.render === "function"
      ) {
        captchaWidgetId = turnstile.render($container[0], {
          sitekey: eqb_vars.captcha.site_key,
        });
        return;
      }
  ...
  function requestCaptchaToken() {
    ...
    if (isRecaptchaV3()) {
      ...
      grecaptcha.ready(function () {
        grecaptcha
          .execute(eqb_vars.captcha.site_key, {
            action: eqb_vars.captcha.action || "eqb_quick_buy",
          })
          .then(function (token) {
    ...
    deferred.resolve(getCaptchaToken());
    return deferred.promise();
  }
  ...
  function loadWards($popup, maTinh, selectedWard) {
    ...
    $ward
      .prop("disabled", true)
      .html('<option value="">' + eqb_vars.i18n.loadingWards + "</option>");
    ...
    $.post(eqb_vars.ajax_url, {
      ...
      .done(function (res) {
        ...
        if (
          selectedWard &&
          $ward.find('option[value="' + selectedWard + '"]').length
        ) {
  ...
  function restoreSavedAddress($popup) {
    ...
    if (
      !savedProvince ||
      !$province.find('option[value="' + savedProvince + '"]').length
    ) {
      return;
    }
  ...
  function bindPopupEvents() {
    ...
    $popup.on(
      "input change",
      '[name="name"], [name="phone"], [name="email"], [name="address"], [name="note"]',
      function () {
    ...
    $form.on("change", "[data-eqb-consent]", function () {
      ...
      if (checked) {
        $form
          .find(".eqb-form__consent")
          .removeClass("eqb-form__consent--error");
        $form
          .find("[data-eqb-message]")
          .addClass("eqb-hidden")
          .removeClass("eqb-form__message--success eqb-form__message--error");
      }
    });

    if ($form.find("[data-eqb-consent]").is(":checked")) {
      $form.find("[data-eqb-consent]").trigger("change");
    }
  ...
  function submitOrder($popup, $form) {
    ...
    if (!$consent.is(":checked")) {
      $msg
        .removeClass("eqb-hidden")
        .addClass("eqb-form__message--error")
        .text(eqb_vars.i18n.consentRequired);
    ...
    var variationId =
      parseInt($form.find("[data-eqb-variation-id]").val(), 10) || 0;

    if (!name || !phone || !maTinh || !maXa) {
      $msg
        .removeClass("eqb-hidden")
        .addClass("eqb-form__message--error")
        .text(eqb_vars.i18n.required);
      return;
    }
    ...
    if (
      ...
      if (variationId <= 0) {
        $msg
          .removeClass("eqb-hidden")
          .addClass("eqb-form__message--error")
          .text(eqb_vars.i18n.selectVariant);
        return;
      }
    }

    if (isCaptchaWidgetProvider()) {
      var widgetToken = getCaptchaToken();
      if (!widgetToken) {
        $msg
          .removeClass("eqb-hidden")
          .addClass("eqb-form__message--error")
          .text(eqb_vars.i18n.captchaRequired);
        return;
      }
    }
    ...
    $msg
      .removeClass("eqb-hidden eqb-form__message--error")
      .addClass("eqb-form__message--success")
      .text(eqb_vars.i18n.submitting);

    setStoredValue(STORAGE_KEY_PROVINCE, maTinh);
    setStoredValue(STORAGE_KEY_WARD, maXa);
    saveCustomerFromForm($form);

    requestCaptchaToken().done(function (captchaToken) {
      if (isCaptchaEnabled() && !captchaToken) {
        $msg
          .removeClass("eqb-form__message--success")
          .addClass("eqb-form__message--error")
          .text(eqb_vars.i18n.captchaRequired);
      ...
      var postData = {
        ...
        payment_method:
          $form.find('[name="payment_method"]:checked').val() || "",
      };
      ...
      if (isCaptchaEnabled()) {
        postData.eqb_captcha_token = captchaToken;
        postData.eqb_captcha_provider = eqb_vars.captcha.provider;
      }

      if (eqb_vars.debug_note === "1") {
        postData.debug_page_url = window.location.href;
        postData.debug_referrer = document.referrer || "";
        postData.debug_screen = window.screen
    ...
    $.post(eqb_vars.ajax_url, {
      ...
      .done(function (res) {
        if (!res.success || !res.data.html) {
          $content.html(
            '<p class="eqb-form__message eqb-form__message--error">' +
              eqb_vars.i18n.error +
              "</p>",
          );
          return;
